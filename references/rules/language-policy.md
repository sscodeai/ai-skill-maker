...
Write reusable agent instructions in English so future AI coding agent sessions can reliably execute them. Ask interview questions and generate skill-facing or artifact-facing reference content in the user's selected language and locale profile when that improves usefulness.
...
Start each new-skill interview by asking the author's comfortable language and whether they want the rest of the interview and generated references to use that language. If yes, continue the interview in that language and bias generated references toward that language and locale profile. If no, ask the author to choose the interview/output languages explicitly. Default to English when the author has no preference.

## Language Fields to Capture

- `instruction_language`: English by default for `SKILL.md` and procedural agent instructions.
- `reference_language`: language for generated reference files.
- `artifact_language`: language for user-facing artifacts produced by the generated skill.
...
- `use_comfort_language_for_generated_references`: whether to bias generated references toward the author's comfortable language.
...
- If the author does not want to use their comfortable language for the skill, ask them to choose `developer_interview_language`, `reference_language`, `artifact_language`, and `public_voice_language`; default unresolved fields to English.
- Preserve product names, command names, API identifiers, paths, and code symbols in their original language.
- If the user mixes languages, ask whether generated references should be bilingual or primarily one language.
- For multilingual skills, record which language belongs to references, artifact output, UI text, examples, comments, and release notes.

# {{projectDisplayName}} Trigger Tests
...
| --- | --- | --- | --- |
| Maintain {{projectName}} by making a focused, verified code change. | yes | | |
| Summarize an unrelated repository without using {{projectName}} context. | no | | |
| Review {{projectName}} for risks before a release. | yes | | |
...
- declared_intent: the skill owner decides which project requests are in or out of scope.

<!-- BEGIN USER RULES -->
<!-- Add durable project trigger-test rules here. This block is preserved on refresh. -->
<!-- END USER RULES -->
